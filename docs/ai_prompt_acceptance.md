...
| A3 | 删除未接线备用 AI 链路 | 仓库只保留唯一生效的 AI 诊断主链路，避免双实现漂移 | `modules/ai_analyzer.py` | 已完成 |
...
| A6 | 建议与心理学报告口径统一 | 规则建议、心理学报告、AI 分析三者不要互相打架 | `modules/advisor.py`, `modules/reference_pipeline.py`, `modules/ai_analyzer.py` | 已完成 |
...
- Prompt 中说明常见特征与理论的关联方式，例如：
  ...
  - `edge_density` / `color_saturation` 可辅助判断信息噪音与视觉唤醒
...
- 仓库中不再保留未接线、未生效的第二套 AI 诊断实现。
- `/ai-analysis` 的运行时入口只对应一套清晰可维护的实现。
- 后续修改 AI 文案时，不会出现“改了错误文件但线上无变化”的情况。
...
| 2026-04-07 | 重写主链路语气提示词，并重写心理学规则报告 | `modules/ai_analyzer.py`, `modules/reference_pipeline.py`, `docs/ai_prompt_acceptance.md` | A4, A5 | 已完成代码替换与语法检查 | A5 当前仅部分落地，新增的 0.12 / 0.15 / 0.25 等阈值后续仍需统一收口 |
...
| 2026-04-07 | 清理当前主链路已失效的遗留字段与规则分支，统一规则引擎、AI 提示词和前端类型到 5 个真实特征 | `config.py`, `modules/advisor.py`, `modules/ai_analyzer.py`, `front/src/api/analyze.ts`, `docs/ai_prompt_acceptance.md` | A1, A5, A6, A8 | Python 语法检查通过，前端 `pnpm build` 通过 | 删除 `brightness / contrast / saturation` 的遗留引用，并移除依赖 `ctr.percentile` 的休眠建议分支 |
| 2026-04-07 | 删除未接线的英文备用 AI 链路，并移除误导性的 `CLIP_DEVICE` 与 CTR 百分位兼容字段 | `api.py`, `config.py`, `modules/ai_analysis.py`, `front/src/api/analyze.ts`, `README.md`, `docs/ai_prompt_acceptance.md` | A3, A5, A8 | Python 语法检查通过，前端 `pnpm build` 通过 | 仓库收口为单一 AI 实现，`/analyze` 的 CTR 返回只保留实际生效字段 |
