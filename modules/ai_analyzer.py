...
- 关联指标：主体占比 (subject_area_ratio)、颜色饱和度 (color_saturation)
- 解读逻辑：人类视觉系统在信息流中会自动被显著性区域吸引（自下而上的加工机制）。主体占比不足或色彩唤醒度偏弱时，用户更难在第一眼锁定单一焦点，视觉注意更容易被背景噪音分流。
...
FEATURE_CONTEXT = f"""各指标含义、参考区间及认知心理学关联：

- 信息熵 (entropy): 衡量图像视觉复杂度，范围 0-8，一般好的主图在 5-7
  → 认知负荷理论：过高（>{config.ENTROPY_HIGH}）触发认知过载；过低（<{config.ENTROPY_LOW}）缺少视觉吸引力
  → 信息过载理论：与 text_density 叠加时加剧决策瘫痪

- 文字密度 (text_density): 图上文字占比，范围 0-1，超过 {config.TEXT_DENSITY_HIGH} 说明文字过多
  → 信息过载理论：密集文案制造信息噪音，干扰核心卖点识别
  → 认知负荷理论：占比过高增加工作记忆负担

- 主体占比 (subject_area_ratio): 范围 0-1，低于 {config.SUBJECT_AREA_RATIO_LOW} 主体太小
